        ...
        <section class="col-xs-12">
            <div class="headline-row">
                <h1 class="section-title caticon sbx">
                    <img src="{{ asset('images/brand_small.png') }}"> Editar usuário - {{ $item->nome }}
                </h1>
            </div>
        </section>
    </div>
            ...
            <div class="box {{ $box }} box-solid">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group {{ $errors->has('nivel_acesso') ? 'has-error' : '' }}">
                        ...
                        {!! Form::select('nivel_acesso' , \App\User::$mapAcesso , $item->nivel_acesso, ['placeholder' => 'Selecione uma opção','class' => 'form-control']) !!}
                    </div>
                    <div class="form-group {{ $errors->has('grupo_id') ? 'has-error' : '' }}">
                        {!! Form::label('grupo_id', 'Grupo de Permissões') !!}
                        {!! Form::select('grupo_id' , \App\ACL\Grupo::lists('nome', 'id') , $item->grupo_id, ['placeholder' => 'Selecione uma opção','class' => 'form-control']) !!}
                    </div>
                    <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                        {!! Form::label('status', 'Status') !!}
                        {!! Form::select('status' , \App\User::$mapStatus , $item->status, ['placeholder' => 'Selecione uma opção','class' => 'form-control']) !!}
                    </div>
                    <div class="form-group">
                        {!! Form::label('loja_id', 'Franquia') !!}
                        {!! Form::select('loja_id[]' , \App\Models\Loja::lists('nome', 'id')->sortBy('nome') , $item->lojas->pluck('id')->toArray(), ['class' => 'form-control select2', 'multiple' => true]) !!}
                    </div>
                    <div class="form-group {{ $errors->has('password') ? 'has-error' : '' }}">
                        {!! Form::label('password', 'Senha') !!}
                        {!! Form::password('password' , ['placeholder' => 'Deixe em branco para não modificar', 'class' => 'form-control']) !!}
                    </div>
                    <div class="form-group {{ $errors->has('password') ? 'has-error' : '' }}">
                        {!! Form::label('password_confirmation', 'Confirme a senha') !!}
                        {!! Form::password('password_confirmation' , ['placeholder' => 'Deixe em branco para não modificar','class' => 'form-control']) !!}
                    </div>
                </div>

        ...
        <section class="col-xs-12">
            <div class="headline-row">
                <h1 class="section-title caticon sbx">
                    <img src="{{ asset('images/brand_small.png') }}"> Nova Loja
                </h1>
            </div>
        </section>
    </div>
            ...
            <div class="box {{ $box }} box-solid">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group {{ $errors->has('praca_id') ? 'has-error' : '' }}">
                        ...
                        {!! Form::select('praca_id' , \App\Models\Praca::lists('nome', 'id') , '', ['placeholder' => 'Selecione uma opção','class' => 'form-control']) !!}
                    </div>
                    <div class="form-group {{ $errors->has('cep') ? 'has-error' : '' }}">
                        {!! Form::label('cep', 'CEP') !!}
                        <div class="row">
                            <div class="col-sm-10">
                                {!! Form::text('cep', '', ['class' => 'form-control', 'data-mask' => '99999-999']) !!}
                            </div>
                            <div class="col-sm-2">
                    ...
                    <div class="form-group {{ $errors->has('uf') ? 'has-error' : '' }}">
                        {!! Form::label('uf', 'UF') !!}
                        {!! Form::text('uf', '', ['class' => 'form-control', 'maxlength' => 2, 'readonly']) !!}
                    </div>
                    <div class="form-group {{ $errors->has('cidade') ? 'has-error' : '' }}">
                        {!! Form::label('cidade', 'Cidade') !!}
                        {!! Form::text('cidade', '', ['class' => 'form-control', 'readonly']) !!}
                    </div>
                    <div class="form-group {{ $errors->has('bairro') ? 'has-error' : '' }}">
                        {!! Form::label('bairro', 'Bairro') !!}
                        {!! Form::text('bairro', '', ['class' => 'form-control', 'readonly']) !!}
                    </div>
                    <div class="form-group {{ $errors->has('endereco') ? 'has-error' : '' }}">
                        {!! Form::label('endereco', 'Endereço') !!}
                        {!! Form::text('endereco', '', ['class' => 'form-control', 'readonly']) !!}
                    </div>
                    ...
                    <div class="form-group {{ $errors->has('valor_minimo_pedido') ? 'has-error' : '' }}">
                        ...
                        {!! Form::text('valor_minimo_pedido' , '' , ['class' => 'form-control maskMoney', 'data-affixes-stay' => 'true', 'data-prefix' => 'R$ ', 'data-thousands' => '.', 'data-decimal' => ',']) !!}
                    </div>
                </div>
...
@section('extra_scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
             ...
             $('#cep').blur(function () {
                var cep = $(this).val();
                $.get("https://apps.widenet.com.br/busca-cep/api/cep/" + cep + ".json", function (data) {
                    if (data.status) {
                        $('#uf').val(data.state);

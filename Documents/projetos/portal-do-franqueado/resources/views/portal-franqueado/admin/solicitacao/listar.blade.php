...
@section('content')
    ...
    @include('portal-franqueado.admin.solicitacao.indicadores')
    @if($auth->isAdmin())
        @include('portal-franqueado.admin.solicitacao.ultimas')
    @endif

    <div class="row to-replace">
            ...
            $(function () {
                $('.swal-confirmation .fake-submit').click(function () {
                    ...
                    swal({
                            ...
                            confirmButtonText: "Sim, deletar solicitação",
                ...
                $('#fake-form').submit(function (e) {
                    var value = $('#fake_palavra_chave').val();
                    $('#palavra_chave').val(value);
                    $('#real-filter').submit();
                    return false;
                });


                var $input = $('#palavra-chave');
                var timeout = null;
                ...
                $input.on('keyup', function(e){
                    ...
                    timeout = setTimeout(function(){
                        url = "{{ url('solicitacao') }}" + '?' + $.param({"filter": {"palavra_chave" : $diz.val()}});
                        console.log(url);
                        $.ajax({
                            url: url,
                            "_token": "{{ csrf_token() }}"
                        }).success(function(data){
                            $('.to-replace').html(data);
                        }).error(function(){
                            swal('error');
                        }).done(function(){
                            $('.overlay').hide();

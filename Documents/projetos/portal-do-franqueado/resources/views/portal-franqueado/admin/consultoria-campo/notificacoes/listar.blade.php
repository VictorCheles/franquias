        ...
        <section class="col-xs-12">
            <div class="headline-row">
                <h1 class="section-title caticon sbx">
                    <img src="{{ asset('images/brand_small.png') }}"> Consultoria de Campo - Lista de Notificações
                </h1>
            </div>
        </section>
    </div>
                                    ...
                                    <td class="options">
                                        <div class="btn-group-vertical">
                                            @if($user->hasRoles([\App\ACL\Recurso::ADM_PROGRAMA_QUALIDADE_AVALIACAO_EDITAR]))
                                                <a href="{{ route('admin.consultoria-de-campo.setup.notificacoes.edit', $item->id) }}" class="btn btn-flat btn-default">
                                                    <i class="fa fa-edit"></i> Editar
                                                </a>
                                            @endif
                                            {{--@if($user->hasRoles([\App\ACL\Recurso::ADM_PROGRAMA_QUALIDADE_AVALIACAO_EDITAR]))--}}
                                                {{--<form class="swal-confirmation" action="{{ route('admin.consultoria-de-campo.setup.notificacoes.destroy', $item->id) }}" method="POST" style="display: inline;">--}}
